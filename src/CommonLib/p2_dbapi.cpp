...
bool user_registration_info(TCHAR* inUserPasswd, TCHAR* inNickName, TCHAR* inSignature, int64 inBirthday, TCHAR* inSex, TCHAR* inICON, TCHAR* inProfile, int64 inQQ, TCHAR* inEmail, int64 inPhone, TCHAR* inWallpaper, int32* outID, int64* outPP)
{
    int32 id = 0;
    int64 pp = time(0)/* ppºÅÉú³ÉËã·¨ */;
    DB_HANDLE hConn = DBConnectionPoolAcquireConnection();
    DB_STATEMENT hStmt = DBPrepare(hConn, _T("INSERT INTO user_info (pp,password,nickname,signature,birthday,sex,icon,profile,qq,email,phone,wallpaper,last_login,date_joined,is_locked,try_times,user_status_id,encrypt_repo_id,is_deleted) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"));
    int64 lastLogin = chrono::system_clock::now().time_since_epoch().;
    ...
    DBBind(hStmt, 4, DB_SQLTYPE_VARCHAR, inSignature, DB_BIND_STATIC);
    ...
    DBBind(hStmt, 16, DB_SQLTYPE_INTEGER, 0);
    DBBind(hStmt, 17, DB_SQLTYPE_INTEGER, userStatusID);
    DBBind(hStmt, 18, DB_SQLTYPE_INTEGER, encryptRepoID);
    DBBind(hStmt, 19, DB_SQLTYPE_INTEGER, 0);
...
int user_login(int64 inUserPP, TCHAR* inUserPassword, UserInfo* outUserInfo)
{
	DB_HANDLE hConn = DBConnectionPoolAcquireConnection();
	TCHAR query[256];
	_sntprintf(query, 256, _T("SELECT * FROM user_info WHERE pp=%I64d"), inUserPP);
	DB_RESULT hResult = DBSelect(hConn, query);
	if (!hResult)
	{
		DBConnectionPoolReleaseConnection(hConn);
	...
	outUserInfo->id = DBGetFieldLong(hResult, 0, 0);
	outUserInfo->pp = DBGetFieldInt64(hResult, 0, 1);
	outUserInfo->passwd = DBGetField(hResult, 0, 2, nullptr, 0);
	outUserInfo->nickname = DBGetField(hResult, 0, 3, nullptr, 0);
	outUserInfo->signature = DBGetField(hResult, 0, 4, nullptr, 0);
	outUserInfo->birthday = DBGetFieldInt64(hResult, 0, 5);
	outUserInfo->sex = DBGetField(hResult, 0, 6, nullptr, 0);
	outUserInfo->icon = DBGetField(hResult, 0, 7, nullptr, 0);
	outUserInfo->profile = DBGetField(hResult, 0, 8, nullptr, 0);
	outUserInfo->qq = DBGetFieldInt64(hResult, 0, 9);
	outUserInfo->email = DBGetField(hResult, 0, 10, nullptr, 0);
	outUserInfo->phone = DBGetFieldInt64(hResult, 0, 11);
	outUserInfo->wallpaper = DBGetField(hResult, 0, 12, nullptr, 0);
	outUserInfo->last_login = DBGetFieldInt64(hResult, 0, 13);
	outUserInfo->date_joined = DBGetFieldInt64(hResult, 0, 14);
	outUserInfo->is_locked = DBGetFieldLong(hResult, 0, 15);
	outUserInfo->try_times = DBGetFieldLong(hResult, 0, 16);
	outUserInfo->user_status_id = DBGetFieldLong(hResult, 0, 17);
	outUserInfo->encrypt_repo_id = DBGetFieldLong(hResult, 0, 18);
	outUserInfo->is_deleted = DBGetFieldLong(hResult, 0, 19);
	DBConnectionPoolReleaseConnection(hConn);
	if (outUserInfo->pp != inUserPP)
		return LOGIN_WRONG_PP;
	else if (_tcscmp(outUserInfo->passwd, inUserPassword) != 0)
